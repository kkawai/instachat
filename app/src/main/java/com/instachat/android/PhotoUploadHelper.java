...
import com.instachat.android.api.CloudVisionApi;
...
public class PhotoUploadHelper {
    ...
    public void launchCamera(boolean isChoose) {
        ...
        if (!isChoose) {
            ...
            if (!EasyPermissions.hasPermissions(mActivity, perm)) {
                EasyPermissions.requestPermissions(mActivity, mActivity.getString(R.string.rationale_storage), RC_TAKE_PICTURE, perm);
                return;
            }
        } else {
            String perm = android.Manifest.permission.READ_EXTERNAL_STORAGE;
            if (!EasyPermissions.hasPermissions(mActivity, perm)) {
                EasyPermissions.requestPermissions(mActivity, mActivity.getString(R.string.rationale_storage), RC_CHOOSE_PICTURE, perm);
                return;
            }
        }
    ...
    private void setupTargetFile() {
        ...
        mTargetFileUri = FileProvider.getUriForFile(mActivity, mActivity.getPackageName() + ".fileprovider", mTargetFile);
    ...
    private CloudVisionApi.CloudVisionApiListener mCloudVisionApiListener = new CloudVisionApi.CloudVisionApiListener() {
        @Override
        public void onImageInspectionCompleted(boolean isCallFailed, final boolean isPossiblyAdult, final boolean isPossiblyViolent) {
    ...
    private void uploadFromUri(Uri fileUri, final boolean isPossibleAdult, final boolean isPossibleViolence) {
        ...
        photoRef.putFile(fileUri).addOnProgressListener(new OnProgressListener<UploadTask.TaskSnapshot>() {
            ...
            public void onProgress(UploadTask.TaskSnapshot taskSnapshot) {
                if (mActivityState == null || mActivityState.isActivityDestroyed())
                    return;
                mListener.onPhotoUploadProgress((int) taskSnapshot.getTotalByteCount() / 1024, (int) taskSnapshot.getBytesTransferred() / 1024);
            }
