...
public final class Preferences {
	...
	private static final String PREFERENCE_CURRENT_USER = "user";
	...
	public void clearUser(Editor editor) {
		editor.remove(PREFERENCE_CURRENT_USER);
	}
	...
	public JSONObject getUser() {
		if (!mPrefs.contains(PREFERENCE_CURRENT_USER)) {
			return null;
		}
		try {
			JSONObject user = new JSONObject(mPrefs.getString(
					PREFERENCE_CURRENT_USER, null));
			return user;
	...
	public void saveUser(JSONObject user) {
		...
		if (user == null) {
			...
			editor.putString(PREFERENCE_CURRENT_USER, user.toString());
		}
		editor.commit();
	}
